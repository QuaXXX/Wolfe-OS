...
export const FlashcardDeckModal = ({ 
  ...
  const [topic, setTopic] = useState("Exam High-Yield Concepts");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      handleGenerateDeck(initialCourse, topic);
    }
  }, [isOpen, initialCourse]);

  const handleGenerateDeck = async (cCode = courseCode, cTopic = topic) => {
  ...
  const handleExportToObsidian = async () => {
    ...
    try {
      ...
      const markdownContent = `# ${courseCode} Flashcards - ${topic}\n*Generated by Wolfe OS on ${new Date().toLocaleDateString()}*\n\n` +
        cards.map((c, i) => `### Card ${i + 1}: ${c.concept || 'Concept'}\n**Q**: ${c.front}\n\n**A**: ${c.back}\n\n---`).join('\n\n');
    ...
    <AnimatePresence>
      <div className="fixed inset-0 top-0 left-0 w-screen h-screen z-[100] flex items-center justify-center p-4 select-none">
        ...
        <motion.div
          ...
          className="relative w-full max-w-xl bg-[#0b0e18]/95 border border-white/15 rounded-3xl p-6 sm:p-7 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] backdrop-blur-2xl z-10 space-y-4"
        >
          {/* Header */}
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            <div className="flex items-center gap-3">
              <div 
                className="w-10 h-10 rounded-2xl flex items-center justify-center bg-blue-500/10 border border-blue-500/30 text-blue-400"
              >
                <Layers className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-bold text-white tracking-tight flex items-center gap-2">
                  <span>{courseCode} Active Recall Flashcards</span>
                ...
                <p className="text-xs text-slate-400">High-yield concept mastery generated from your notes</p>
              </div>
            </div>
          ...
          {/* Card View or Completion Screen */}
          {isGenerating ? (
            ...
            <div className="space-y-4">
              ...
              {/* Anki Spaced Repetition Response Buttons (When Flipped) */}
              {isFlipped ? (
