...
export const NotebookLMStudyHubModal = ({ 
  ...
  // Local instant parsing fallback so user NEVER waits for tabs to render
  const localBriefingFallback = useMemo(() => {
    // Find syllabus text in currentCourseFiles
    let matchedText = '';
    for (const f of currentCourseFiles) {
      if (f.cachedContent) matchedText += f.cachedContent + ' ';
    }

    const instructorInfo = extractInstructorFromOutline(matchedText);

    // Extract grade percentages using regex
    const gradeItems = [];
    const weightMatches = matchedText.matchAll(/([A-Za-z\s]{3,25})\s*[:\-–]?\s*(\d{1,2}%)/g);
    for (const m of weightMatches) {
      const title = m[1].trim();
      if (!title.toLowerCase().includes('grade') && !title.toLowerCase().includes('total') && title.length < 30) {
        gradeItems.push({ item: title, weight: m[2], details: "From course outline" });
      }
    }

    const finalGrades = gradeItems.length > 0 ? gradeItems.slice(0, 4) : [
      { item: "Midterm Examination", weight: "30%", details: "Evaluates core course concepts" },
      { item: "Assignments & Case Studies", weight: "25%", details: "Regular semester deliverables" },
      { item: "Final Examination", weight: "35%", details: "Comprehensive registrar scheduled" },
      { item: "Quizzes / Participation", weight: "10%", details: "Ongoing weekly engagement" }
    ];

    return {
      overview: `${selectedCourse} focuses on university academic principles, high-yield conceptual mastery, and analytical problem-solving.`,
      instructor: {
        name: instructorInfo.name || "Professor",
        email: instructorInfo.email || `${selectedCourse.toLowerCase().replace(/[^a-z0-9]/g, '')}@university.edu`,
        officeHours: "By appointment / Check syllabus",
        section: instructorInfo.section || "L01"
      },
      gradeBreakdown: finalGrades,
      keyDates: [
        { title: "Midterm Assessment", date: "Mid-Semester", type: "Exam" },
        { title: "Final Examination", date: "Registrar Scheduled", type: "Final" }
      ],
      highYieldConcepts: [
        { topic: "Core Principles & Frameworks", summary: `Primary conceptual definitions and methodologies for ${selectedCourse}.`, formula: "" },
        { topic: "Applied Problem Solving", summary: "Calculations, analysis models, and exam questions.", formula: "" },
        { topic: "Final Integration", summary: "Cross-topic synthesis for final exam readiness.", formula: "" }
      ],
      examTraps: [
        "Review all lecture slides and course announcements weekly on D2L.",
        "Practice problem sets and definitions without relying on notes."
      ]
    };
  }, [currentCourseFiles, selectedCourse]);
  ...
  const currentBriefing = briefings[selectedCourse] || localBriefingFallback;
  ...
  const modalContent = (
    <div className="fixed inset-0 top-0 left-0 w-screen h-screen z-[100] flex items-center justify-center p-3 sm:p-5">
      ...
      <div
        ...
        className="fixed inset-0 top-0 left-0 w-full h-full bg-black/75 backdrop-blur-xl z-0 cursor-pointer"
      />
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-950/20 to-indigo-950/20 border border-purple-500/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                {currentBriefing.instructor && (
                  <div className="p-3 rounded-xl bg-white/[0.03] border border-white/10 shrink-0 text-xs space-y-0.5 min-w-[180px]">
