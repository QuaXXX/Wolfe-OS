...
export const GoogleCalendarModal = ({ 
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // 2. Immediately trigger 2-way cloud sync across all hubs (Nutrition, Calendar, Settings)
      ...
      if (cloudRes?.success) {
        setSyncMessage("Connected and synced!");
      }
    } catch (err) {
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      playSound('success', soundEnabled);
      refreshStatus();
      setSyncMessage(`Synced ${events ? events.length : 0} calendar items!`);
      if (onSyncSuccess) {
          ...
          <div className="flex items-center justify-between pb-3.5 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Calendar & data sync</p>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-slate-400 font-mono truncate max-w-[200px] sm:max-w-[260px]">
                      {account?.email || 'Connected'}
                    </div>
                  </div>
                </div>

                <span className={`flex items-center gap-1.5 text-[10px] font-mono font-semibold px-2.5 py-1 rounded-xl border shrink-0 ${
                  (error || syncStatus === 'failed' || syncStatus === 'error')
              ...
              {(error || syncStatus === 'failed' || syncStatus === 'error') && (
                <div className="text-xs text-rose-300 bg-rose-500/10 p-3 rounded-xl border border-rose-500/25 space-y-1">
                  ...
                  <p className="text-[11px] leading-relaxed text-rose-300/90">
                    Your Google session requires attention. Tap below to reconnect.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2 pt-1 border-t border-white/10">
                ...
                <div className="flex items-center justify-end pt-1">
                  <button
                    ...
                    className="py-1.5 px-3 rounded-xl bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-300 text-[11px] font-medium border border-white/10 transition-colors cursor-pointer flex items-center gap-1.5"
            ...
            <div className="p-6 rounded-2xl bg-[#131728] border border-white/10 space-y-4 text-center">
              <div className="space-y-1 text-center">
                ...
                <p className="text-xs text-slate-400 max-w-xs mx-auto leading-relaxed">
                  Keep your calendar and data synchronized across your devices.
                </p>
              </div>
              ...
              <button
                ...
                onClick={handleGoogleSignIn}
                disabled={isSyncing}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-2xl bg-white hover:bg-slate-100 active:scale-[0.98] text-slate-900 font-semibold text-xs shadow-xl transition-all disabled:opacity-50 cursor-pointer"
              >
          ...
          <div className="pt-1 border-t border-white/10">
            ...
            {showAdvanced && (
              ...
              >
                ...
                {/* Cloud Master Controls */}
                <div className="p-3 rounded-xl bg-white/[0.02] border border-white/10 space-y-2">
                  <span className="text-[11px] font-bold text-white block">Cloud Master Backup:</span>
                  ...
                  <button
                    ...
                    disabled={isSyncing}
                    className="w-full py-1.5 px-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[11px] font-medium border border-red-500/20 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <Trash2 className="w-3 h-3 shrink-0" />
                    <span>Reset Account Cloud Data</span>
                  </button>
                </div>
              </motion.div>
            )}
